...
class VGGAutoEncoder(nn.Module):
    ...
    def __init__(self, configs, img_size=224):
...
class VGG(nn.Module):
    ...
    def __init__(self, configs, num_classes=1000, img_size=224, enable_bn=False):
        ...
        # Adjust feature sizes based on img_size
        if img_size == 32:  # For CIFAR-100
            self.fc = nn.Sequential(
                nn.Linear(in_features=int(self.img_size*self.img_size*512), out_features=512),
                nn.Dropout(p=0.5),
